...
class ResidualAttentionBlock(nn.Module):
    def __init__(self, d_model: int, n_head: int, attn_mask: torch.Tensor = None, add_adapter=False):
        super().__init__()
        ...
        if add_adapter:
            self.forgery_aware_adapter = ForgeryAwareAdapterLayer()
        else:
...
class Transformer(nn.Module):
    def __init__(self, width: int, layers: int, heads: int, attn_mask: torch.Tensor = None, add_adapter=None):
        super().__init__()
        self.width = width
        self.layers = layers
        if add_adapter is None:
            add_adapter = [False] * layers
        self.resblocks = nn.Sequential(*[ResidualAttentionBlock(width, heads, attn_mask, add_adapter[idx]) for idx in range(layers)])

    def forward(self, x: torch.Tensor):
        out = {}
        for idx, layer in enumerate(self.resblocks.children()):
            x = layer(x)
            out['layer'+str(idx)] = x # shape:LND. choose cls token feature   
        return out, x 
...
class VisionTransformer(nn.Module):
    def __init__(self, input_resolution: int, patch_size: int, width: int, layers: int, heads: int, output_dim: int, args=None):
        ...
        self.transformer = Transformer(width, layers, heads, add_adapter=add_adapter)
